...
public class Mo_ClimbCeiling : Motion {
	...
	void PressingSpacebar(){
		...
		if (Input.GetKeyUp (KeyCode.Space) && _holdingSpace) {
			_holdingSpace = false;
			Jump(); 
		}
	...
	void ApplySpeed(){
		if(!_holdingSpace){
			...
			Vector3 _speedOnPlane = Vector3.ProjectOnPlane((_speed.x*_player.transform.right +_speed.z *_player.transform.forward), _player.GroundHit.normal).normalized;
			_player.Rigid.velocity = _speedOnPlane; 
			//Vector3 _horizontalSpeed = _speed.x * _right;
			//Vector3 _verticalSpeed = _speed.z * _up; 
